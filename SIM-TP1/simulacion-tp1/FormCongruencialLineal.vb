Public Class FormCongruencialLineal
    Private this As Object

    Private Sub FormCongruencialLineal_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        limpiar()
        txtN.Text = "20"
    End Sub

    Private Sub btnGenerar_Click(sender As Object, e As EventArgs) Handles btnGenerar.Click
        If Not (txtXo.Text = "") And Not (txtK.Text = "") And Not (txtG.Text = "") And Not (txtC.Text = "") Then
            If IsNumeric(txtXo.Text) And IsNumeric(txtA.Text) And IsNumeric(txtC.Text) And IsNumeric(txtM.Text) Then
                If txtXo.Text > 0 And txtA.Text > 0 And txtC.Text > 0 And txtM.Text > 0 Then
    ...
    Private Sub generar()
        If Not (txtXo.Text = "") And Not (txtK.Text = "") And Not (txtG.Text = "") And Not (txtC.Text = "") Then
            ...
            For i = 1 To n Step 2
                ...
                'Me.dgvResultados.Rows.Add(i, a * xi + c, xs, Trim(Microsoft.VisualBasic.Left(rndi1s, 6)), Trim(Microsoft.VisualBasic.Left(rndi2s, 6)))
                xi = xs
            Next
            xi = x0
            xs = x0
            For Each Fila As DataGridViewRow In dgvNumeros.Rows
                xs = (a * xi + c) Mod m
                Me.dgvResultados.Rows.Add(Fila.Cells(0).Value, a * xi + c, xs, Fila.Cells(1).Value)
                xi = xs
            Next
        Else
            Dim n As New Integer
    ...
    Private Sub txtK_TextChanged(sender As Object, e As EventArgs) Handles txtK.TextChanged
        If txtK.Text <> "" Then
            txtA.Text = (1 + 4 * (CInt(txtK.Text))).ToString()
        End If
    End Sub

    Private Sub FormCongruencialLineal_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        If MessageBox.Show("żQuiere cerrar?", "Importante", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            e.Cancel = False
        Else
            e.Cancel = True
        End If
    ...
    Private Sub btnGrafico2_Click(sender As Object, e As EventArgs) Handles btnGrafico2.Click
        If txtXo.Text = "" And txtK.Text = "" And txtG.Text = "" And txtC.Text = "" Then
            ...
            For Each Fila As DataGridViewRow In dgvFrecuencias.Rows
                histograma.Series("Frecuencia Observada").Points.AddXY(Fila.Cells(0).Value, Fila.Cells(1).Value)
                histograma.Series("Frecuencia Esperada").Points.AddXY(Fila.Cells(0).Value, Fila.Cells(2).Value)
            Next
        End If
    End Sub
    ...
    Private Sub btnChi_Click(sender As Object, e As EventArgs) Handles btnChi.Click
        ...
        dgvFrecuencias.Columns(2).DefaultCellStyle.Format = "N6"
        dgvFrecuencias.Columns(3).DefaultCellStyle.Format = "N4"
        dgvFrecuencias.Columns(4).DefaultCellStyle.Format = "N4"
        If Not (txtXo.Text = "") And Not (txtK.Text = "") And Not (txtG.Text = "") And Not (txtC.Text = "") Then
            Dim x0 As New Integer
            x0 = CInt(txtXo.Text)
            Dim k As New Integer
            ...
            If cmbIntervalos.SelectedIndex <> -1 Then
                ...
                Dim varianza As New Double
                Dim total As New Double
                Dim totalVarianza As New Double

                For Each Fila As DataGridViewRow In dgvResultados.Rows
                    total += Fila.Cells(3).Value
                Next
                ...
                For Each Fila As DataGridViewRow In dgvNumeros.Rows
                    If intervalos = 5 Then
                        If Fila.Cells(1).Value >= 0 And Fila.Cells(1).Value < 0.2 Then
                            dgvFrecuencias.Rows(0).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.2 And Fila.Cells(1).Value < 0.4 Then
                            dgvFrecuencias.Rows(1).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.4 And Fila.Cells(1).Value < 0.6 Then
                            dgvFrecuencias.Rows(2).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.6 And Fila.Cells(1).Value < 0.8 Then
                            dgvFrecuencias.Rows(3).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.8 And Fila.Cells(1).Value <= 1 Then
                            dgvFrecuencias.Rows(4).Cells(1).Value += 1
                        End If
                    End If

                    If intervalos = 10 Then
                        If Fila.Cells(1).Value >= 0 And Fila.Cells(1).Value < 0.1 Then
                            dgvFrecuencias.Rows(0).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.1 And Fila.Cells(1).Value < 0.2 Then
                            dgvFrecuencias.Rows(1).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.2 And Fila.Cells(1).Value < 0.3 Then
                            dgvFrecuencias.Rows(2).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.3 And Fila.Cells(1).Value < 0.4 Then
                            dgvFrecuencias.Rows(3).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.4 And Fila.Cells(1).Value < 0.5 Then
                            dgvFrecuencias.Rows(4).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.5 And Fila.Cells(1).Value < 0.6 Then
                            dgvFrecuencias.Rows(5).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.6 And Fila.Cells(1).Value < 0.7 Then
                            dgvFrecuencias.Rows(6).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.7 And Fila.Cells(1).Value < 0.8 Then
                            dgvFrecuencias.Rows(7).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.8 And Fila.Cells(1).Value < 0.9 Then
                            dgvFrecuencias.Rows(8).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.9 And Fila.Cells(1).Value <= 1 Then
                            dgvFrecuencias.Rows(9).Cells(1).Value += 1
                        End If
                    End If

                    If intervalos = 15 Then
                        If Fila.Cells(1).Value >= 0 And Fila.Cells(1).Value < 0.0667 Then
                            dgvFrecuencias.Rows(0).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.0667 And Fila.Cells(1).Value < 0.1334 Then
                            dgvFrecuencias.Rows(1).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.1334 And Fila.Cells(1).Value < 0.2001 Then
                            dgvFrecuencias.Rows(2).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.2001 And Fila.Cells(1).Value < 0.2668 Then
                            dgvFrecuencias.Rows(3).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.2668 And Fila.Cells(1).Value < 0.3335 Then
                            dgvFrecuencias.Rows(4).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.3335 And Fila.Cells(1).Value < 0.4002 Then
                            dgvFrecuencias.Rows(5).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.4002 And Fila.Cells(1).Value < 0.4669 Then
                            dgvFrecuencias.Rows(6).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.4669 And Fila.Cells(1).Value < 0.5336 Then
                            dgvFrecuencias.Rows(7).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.5336 And Fila.Cells(1).Value < 0.6003 Then
                            dgvFrecuencias.Rows(8).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.6003 And Fila.Cells(1).Value < 0.667 Then
                            dgvFrecuencias.Rows(9).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.667 And Fila.Cells(1).Value < 0.7337 Then
                            dgvFrecuencias.Rows(10).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.7337 And Fila.Cells(1).Value < 0.8004 Then
                            dgvFrecuencias.Rows(11).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.8004 And Fila.Cells(1).Value < 0.8671 Then
                            dgvFrecuencias.Rows(12).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.8671 And Fila.Cells(1).Value < 0.9338 Then
                            dgvFrecuencias.Rows(13).Cells(1).Value += 1
                        End If
                        If Fila.Cells(1).Value >= 0.9338 And Fila.Cells(1).Value <= 1 Then
                            dgvFrecuencias.Rows(14).Cells(1).Value += 1
                        End If
                    End If
                Next
                ...
                For Each Fila As DataGridViewRow In dgvFrecuencias.Rows
                    totalVarianza += ((Fila.Cells(1).Value - Fila.Cells(2).Value) ^ 2) / Fila.Cells(2).Value
                Next


                varianza = totalVarianza

                Dim m As New Integer
                If media <> 0 And varianza <> 0 Then
                    m = 2
                End If
                ...
                If varianza <> 0 And media = 0 Then
                    m = 1
                End If
                ...
                txtMedia.Text = Trim(Microsoft.VisualBasic.Left(media, 6))
                txtVarianza.Text = varianza
                txtGradosLibertad.Text = v
            Else
